...
const STATUS_CONFIG: Record<DealStatus, { icon: React.ReactNode; text: string; bg: string; glow: string }> = {
  Created:   { icon: <Clock className="w-3 h-3" />,          text: "text-surface-800", bg: "bg-surface-400/15", glow: "" },
  Funded:    { icon: <Wallet className="w-3 h-3" />,         text: "text-blue-400",    bg: "bg-blue-500/10",    glow: "shadow-[0_0_8px_rgba(59,130,246,0.15)]" },
  Delivered: { icon: <Send className="w-3 h-3" />,           text: "text-amber-400",   bg: "bg-amber-500/10",   glow: "shadow-[0_0_8px_rgba(245,158,11,0.15)]" },
  ...
  Disputed:  { icon: <AlertTriangle className="w-3 h-3" />,  text: "text-red-400",     bg: "bg-red-500/10",     glow: "shadow-[0_0_8px_rgba(239,68,68,0.15)]" },
...
// Steps for the mini progress bar
...
export default function DealCard({
  ...
  return (
    <Link href={`/deal/${id}`}>
      <div className={`group relative overflow-hidden rounded-2xl p-5 h-full cursor-pointer transition-all duration-300 bg-surface-100 border border-white/[0.06] hover:border-white/[0.1] hover:shadow-[0_12px_48px_rgba(0,0,0,0.3)] hover:-translate-y-1`}>
        {/* Hover gradient */}
        ...
        <div className="relative">
          ...
          <div className="flex items-start justify-between gap-3 mb-3">
            ...
            <div className={`status-badge ${config.bg} ${config.text} ${config.glow}`}>
              {config.icon}
              {status}
            </div>
          </div>
